...
void ofApp::setup(){
    ...
    */
	...
	gui.add(rate.setup("time stretch", 1, 1, 1000 ));
...
void ofApp::update(){
    ...
    n++;
    t += 1.0 / float(FRAME_RATE);

    //while (n >= exp_arrival) {
    if (t >= exp_arrival_t + exp_wait * dT) {
    ...
    //while (n >= sib_arrival) {
    if (t >= sib_arrival_t + sib_wait *dT) {
...
void ofApp::draw(){
    ...
    ofDrawBitmapString(ofToString(exp_arrival + exp_wait - n), midpoint[0] + exp_pos[0] * dX + 6, midpoint[1] + exp_pos[1] * dX - 3);
    ...
    ofSetColor(ofColor(255));
    ofDrawBitmapString(ofToString(sib_arrival + sib_wait - n), midpoint[0] + sib_pos[0] * dX + 6, midpoint[1] + sib_pos[1] * dX - 3);
